...
public class Badges {
	...
	public static void validateGoldCollected() {
		...
		if (!local.contains( Badge.UNLOCK_PORRIMA ) && Statistics.goldCollected >= 1000) // 골드 1000 이상 획득 시 포리마 해금
		{
			badge = Badge.UNLOCK_PORRIMA;
			local.add( badge );
			validatePorrimaUnlock();
		}
	...
	// 직업 해금
	public static void validatePorrimaUnlock(){ // 직업 - 포리마 잠금 조건
		if (Statistics.goldCollected >= 1000 && !isUnlocked(Badge.UNLOCK_PORRIMA)){
			displayBadge( Badge.UNLOCK_PORRIMA );
		}
	}
